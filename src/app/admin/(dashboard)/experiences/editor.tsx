...
export function ExperiencesEditor({ initial }: { initial: Experience[] }) {
    ...
    <div>
      <div className="mt-6 flex justify-between">
        <p className="text-sm text-muted-foreground">
          {initial.length}{" "}
          {initial.length === 1 ? "experience" : "experiences"}.
        </p>
        {!adding && !editing && (
      ...
      {adding && <ExperienceForm onClose={close} />}
      {editing && <ExperienceForm onClose={close} initial={editing} />}
...
function ExperienceCard({
  ...
  function onDelete() {
    ...
    start(async () => {
      const res = await deleteExperience(experience.slug);
      if (res.ok) toast.success(`Removed ${experience.name}`);
...
function ExperienceForm({
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      const res = initial
        ? await updateExperience(initial.slug, fd)
    ...
    >
      <div className="flex items-center justify-between">
        <h2 className="font-display text-xl font-bold tracking-tight">
          {initial ? `Edit "${initial.name}"` : "Add an experience"}
        </h2>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="grid gap-1.5">
        ...
        <Input
          ...
          placeholder="Yacht Charters, Private Chef…"
          required
        />
      </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={pending} className="rounded-md">
          {pending ? "Saving…" : initial ? "Save changes" : "Add experience"}
        </Button>
      </div>
    </form>
  );
